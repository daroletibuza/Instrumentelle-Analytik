...
\label{gl:volumenstrom}
	D &= \frac{V}{t} = \frac{\SI{0,5}{\liter}}{\SI{48,07}{\second}} = \SI{0,010402}{\liter \per \second} = \underline{\SI{0,624}{\liter \per \minute}}
\end{flalign}

\newpage

\subsection*{Messung der Raumluftprobe}
Mit der nun aufgestellten Kalibriergerade können nun aus gemessenen Absorbanzen (siehe Tab. \ref{tab:probe}) der Raumluftprobe die Konzentrationen bestimmt werden (vgl. Gl.\ref{gl:probe}). Die Konzentration an \ce{NO2} in der Raumluft bestimmt sich mit Hilfe von Gleichung \ref{gl:volumen}
...
\begin{flalign}
	\label{gl:volumen}
	\beta &= \frac{m_A}{D*t}*\frac{T_n+\theta}{T_n+\theta_a} = \frac{\SI{0,153}{\micro\gram}}{\SI{0,624e-3}{\kmeter \per \minute}*\SI{90}{\min}}*\frac{\SI{273}{\kelvin}+\SI{26}{\kelvin}}{\SI{273}{\kelvin}+\SI{24,1}{\kelvin}}\\
...
\end{table}%
\FloatBarrier
Es fällt auf das alle drei Messungen und demzufolge auch der Mittelwert für $\beta$ kleiner sind als der Arbeitsplatzgrenzwert für \ce{NO2} laut Gefahrstoffverordnung ($\beta<$\SI{950}{\micro \gram \per \kmeter}).
